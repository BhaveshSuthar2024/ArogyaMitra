var meSpeak = (function() {
 	...
 	var coreName = 'mespeak-core.js',
		...
		hasWorker = (!isMobile && typeof window !== 'undefined' && typeof window.Worker !== 'undefined'),
		audioUnlocked = false,
		coreReady = false,
		baseUrl,
		worker;
	...
	function coreReceiver(event) {
		var msgType = event.data.rsp, jobId = event.data.jobId;
		switch(msgType) {
			case 'ready':
				try {
					var t=new ArrayBuffer(1);
					worker.postMessage({'job': 'test', 'jobId': 0, 'test': t}, [t]);
					if (!t.byteLength) worker.postMessage({'job': 'useTransferables', 'jobId': 0, 'args': true});
				}
				catch(e) {}
				coreLoaded();
				break;
	...
	// speak related methods & helpers

	function speak(text, args, callback, id) {
		// public
		// returns: numeric 32bit id (or 0 on failure)
		// option "rawdata": return in callback a stream-object in specified format (default: ArrayBuffer, uint8)
		...
		var argstack = [
			'-w', 'wav.wav',
			'-a', (typeof args.amplitude !== 'undefined')? String(args.amplitude) : (typeof args.a !== 'undefined')? String(args.a) : '100',
			...
			'-b', args.utf16 ? '4' : (typeof args.b !== 'undefined')? String(args.b) : '1',
			'-v', varg
		];
		...
		if (hasWorker) {
			sendWorkerJob({'job': 'speak', 'args': argstack}, id, function(stream) {
		...
		for (var i=0; i<parts.length; i++) {
			...
			speak(part.text, opts, function(success, jobId, stream) {
				...
				if (--stackLength===0) {
					...
					var wav, sampleLength = 0, first = true;
					...
					// resolve to callback or playback
	...
	AudioPlayback.prototype= {
		play: function(stream, isDataUrl) {
			try {
				...
				this.adjustVolume();
				var that=this;
				this.audio.addEventListener('ended', function() {that.remove();}, false);
				this.audio.addEventListener('canplaythrough', function() {that.audio.play(); that.playing=true;}, false);
				if (isDataUrl) {
	...
	function playWebSound(stream, volume, callback, id, pan) {
		try {
			var source, gainNode, timer, relVolume, sid = String(id);
			if (!audioContext) {
			...
			if (pan && !isNaN(pan)) {
				// mix up mono to stereo and implement a robust stereoPanner
				try {
			...
			audioContext.decodeAudioData(stream, function(audioData) {
				var f=function() { webSoundEndHandler(source, gainNode, callback, true, id); };
	...
	function webSoundEndHandler(source, gainNode, callback, success, id) {
		...
		// finally clean up with a bit of delay (work around a Chrome duration bug/issue)
		var f=function() {
			if (source) {
	...
	function setFilter() {
		...
		for (var i=0; i< arguments.length; i++) {
			...
			var filter, type = String(data.type).toLowerCase();
			switch (type) {
				case 'lowpass':
				case 'highpass':
				case 'bandpass':
				...
				case 'allpass':
					...
					catch(e1) { if (self.console) console.warn('meSpeak.setFilter(): Failed to create filter "'+type+'", '+e1.message); };
					break;
...
// public API
	...
	function setDefaultVoice(voice) {
		...
		if (voice) {
			if (!coreReady) {
				deferredLoaders.push({'job': setDefaultVoice, 'args': [voice]});
			}
	...
	function getRelFileUrl(url) {
		// workers load files relative from their own locations, instances relative to page location
		// mimic behavior for instances
		if ((/^(\/|https?:|ftps?:|file:)/i).test(url)) return url;
		else {
	...
	function loadVoice(url, callback) {
		...
		if (url && typeof url === 'string') {
			if (coreReady) {
				...
				if ((/^[\w\-]+(\/[\w\-]+)?$/).test(url)) {
					// bare voice-id, map to "voices/<voice-id>.json" relative to script url
					// eg.g., "en/en-us" is loaded from "/path/to/meSpeak/voices/en/en-us.json".
					url = 'voices/' + url + '.json';
				}
				url = getRelFileUrl(url);
				if (hasWorker) {
					sendWorkerJob({'job': 'loadVoice', 'args': [url]}, 0, handler);
				}
				else {
				...
				deferredLoaders.push({'job': loadVoice, 'args': [url, callback]});
			}
		}
	}

	function loadConfig(url) {
		// public (legacy API, now loaded implicitely)
	}
	...
	function loadCustomConfig(url, callback) {
		...
		if (url && typeof url === 'string') {
			if (coreReady) {
				...
				url = getRelFileUrl(url);
				if (hasWorker) {
					sendWorkerJob({'job': 'loadConfig', 'args': [url]}, 0, handler);
				}
				else {
				...
				deferredLoaders.push({'job': loadCustomConfig, 'args': [url, callback]});
			}
		}
	}
	...
	// unlock mobile audio, Safari desktop
	function unlockAudio(event) {
		event.returnValue = true;
		if (audioUnlocked) return;
		if (AudioAPI) {
			// resume, if suspended, and create a gain node
			try {
				if (!audioContext) audioContext=new AudioAPI();
				if (audioContext.state === 'suspended') audioContext.resume();
	...
	if (isMobile && typeof self.ontouchstart !== undefined) self.addEventListener('touchstart', unlockAudio, { 'passive': true, 'capture': true });
